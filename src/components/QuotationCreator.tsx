...
import { createQuotationNumber } from "@/lib/utils";
...
export function QuotationCreator({
  ...
  function updateLabour(
    index: number,
    field: "desc" | "amount" | "discount" | "tax" | "totalWithTax",
    value: string,
  ) {
    setLabourRows((current) =>
      current.map((row, rowIndex) => {
...
const handlePartKeyDown = (e: React.KeyboardEvent) => {
  ...
  async function handleSave(opts = { showPreview: false }) {
    setSaveError("");
  ...
  async function openPreview() {
    await handleSave({ showPreview: true });
  }
            ...
            <div className={styles.searchWrap}>
              <div className={`${styles.searchField} ${partOpen ? styles.searchFocused : ""}`}>
                ...
                <input
                  type="text"
                  value={partQuery}
                  placeholder="Search and add a part…"
