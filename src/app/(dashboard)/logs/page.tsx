...
type LogEntry = {
  id: string;
  type: "vehicle" | "enquiry" | "spare_part" | "spare_order" | "transaction" | "invoice" | "quotation" | "account" | "unknown";
...
const typeConfig = {
  ...
  account: { icon: User, color: "text-cyan-700", bg: "bg-cyan-50", label: "Account" },
  unknown: { icon: Clock, color: "text-slate-500", bg: "bg-slate-100", label: "Activity" },
};

export default function LogsPage() {
  const [logs, setLogs] = useState<LogEntry[]>([]);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState<"all" | "vehicle" | "enquiry" | "spare_part" | "spare_order" | "transaction" | "invoice" | "quotation" | "account">("all");
  ...
  const fetchAllLogs = async () => {
    ...
    try {
      ...
      const mapped: LogEntry[] = ((data || []) as ActivityLogRow[]).map((entry) => ({
        id: entry.id,
        type: (entry.entity_type || "unknown") as LogEntry["type"],
        action: entry.action,
  ...
  return (
    <div className="max-w-[900px] mx-auto py-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex items-center gap-2 mb-8 flex-wrap">
        {(["all", "vehicle", "enquiry", "spare_part", "spare_order", "transaction", "invoice", "quotation", "account"] as const).map((f) => {
        ...
        {loading ? (
          ...
          <div className="divide-y divide-black/[0.03]">
            {filtered.map((log, idx) => {
              const cfg = typeConfig[log.type] || typeConfig.unknown;
              const Icon = cfg.icon;
              return (
