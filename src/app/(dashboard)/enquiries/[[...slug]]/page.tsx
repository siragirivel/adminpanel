...
function EnquiriesContent() {
  ...
  const handleSave = async () => {
    ...
    if (editId) {
      let { error } = await supabase.from("enquiries").update(payload).eq("id", editId);
      if (error && error.message?.toLowerCase().includes("enquiry_date")) {
        const legacyPayload = {
        ...
        const retry = await supabase.from("enquiries").update(legacyPayload).eq("id", editId);
        error = retry.error;
      }
      if (error) {
        alert(error.message);
      ...
      if (error && error.message?.toLowerCase().includes("enquiry_date")) {
        ...
        const retry = await supabase.from("enquiries").insert([
          {
            id: newId,
            ...legacyPayload,
            created_by: auth.user?.id,
  ...
  const handleEdit = (enquiry: EnquiryRow) => {
    ...
    setForm({
      ...
      enquiry_date:
        enquiry.enquiry_date || (enquiry.created_at ? enquiry.created_at.split("T")[0] : new Date().toISOString().split("T")[0]),
        ...
        <div className="min-h-[calc(100vh-40px)] rounded-[28px] border border-slate-200 bg-white shadow-[0_24px_60px_rgba(15,23,42,0.07)] overflow-hidden">
          <div className="px-8 py-6 flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Track customer enquiries, enquiry date, status, and pickup dates.</p>
            </div>
            <button
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    ...
                    <th className="px-6 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Enquiry Date</th>
                      ...
                      <tr key={enquiry.id} onClick={() => router.push(`/enquiries/${enquiry.id}`)} className="group hover:bg-slate-50/50 transition-colors cursor-pointer">
                        ...
                        <td className="px-6 py-4">
                          <p className="text-sm font-medium text-slate-900">
                            {enquiry.enquiry_date || (enquiry.created_at ? enquiry.created_at.split("T")[0] : "—")}
                          </p>
                        </td>
                        <td className="px-6 py-4">
                          <p className="text-sm font-medium text-slate-900">
                            {formatCreatedDateTime(enquiry.created_at)}
                          </p>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4 text-right pr-10">
                          <div className="flex items-center justify-end gap-2">
                            <button
                              ...
                              className="rounded-lg border border-slate-200 px-2.5 py-1 text-[11px] font-semibold text-slate-600 transition-all hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-600  "
      ...
      {isAdding ? (
        <div className="mx-auto max-w-5xl rounded-[28px] border border-slate-200 bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.07)]">
          <div className="mb-8">
            ...
            <p className="mt-1 text-sm text-slate-500">Record customer name, phone, vehicle details, enquiry date, status, and pickup date.</p>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div>
              <label className="mb-2 block text-[12px] font-semibold uppercase tracking-wider text-slate-500">Enquiry date</label>
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(event) => setForm((current) => ({ ...current, enquiry_date: event.target.value }))}
                />
              </div>
            </div>
            <div>
      ...
      {viewEnquiry ? (
        <div className="mx-auto max-w-4xl rounded-[28px] border border-slate-200 bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.07)]">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-200 p-5">
              <div className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">Enquiry date</div>
              <div className="mt-2 text-base text-slate-800">
                {viewEnquiry.enquiry_date || (viewEnquiry.created_at ? viewEnquiry.created_at.split("T")[0] : "—")}
              </div>
            </div>
            <div className="rounded-2xl border border-slate-200 p-5">
            ...
            <div className="rounded-2xl border border-slate-200 p-5">
              ...
              <div className="mt-2 text-base text-slate-800">{formatCreatedDateTime(viewEnquiry.created_at)}</div>
            </div>
            <div className="rounded-2xl border border-slate-200 p-5">
