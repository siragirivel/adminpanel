...
import { cn, createInvoiceNumber, createInvoicePrefix, createQuotationNumber, createVehicleId } from "@/lib/utils";
...
function VehiclesContent() {
  ...
  const [vehicleAdvanceTotal, setVehicleAdvanceTotal] = useState(0);
  const [vehicleInvoiceAdvances, setVehicleInvoiceAdvances] = useState<Record<string, number>>({});
  const [noteDraft, setNoteDraft] = useState("");
  const [savingNote, setSavingNote] = useState(false);
  const [registrationError, setRegistrationError] = useState("");
  const [uploadStatus, setUploadStatus] = useState("");
  const [existingOdometerKm, setExistingOdometerKm] = useState("");
  ...
  useEffect(() => {
    if (!slug || slug.length === 0) {
      ...
      setEditingId(null);
    } else if (slug[0] === 'new' || slug[0] === 'add-new') {
      setViewMode('new');
      setEditingId(null);
      // Try restoring auto-saved draft
      try {
        const saved = localStorage.getItem("sgv_veh_draft");
        if (saved) {
          const d = JSON.parse(saved) as Record<string, unknown>;
          setFormData(prev => ({ ...prev, ...(d.formData as object || {}) }));
          void fetchNextCarId();
          return;
        }
      } catch { localStorage.removeItem("sgv_veh_draft"); }
      resetForm();
      void fetchNextCarId();
  ...
  useEffect(() => {
    if (viewMode !== 'new' || editingId) return;
    window.dispatchEvent(new CustomEvent("sgv:autosave", { detail: { status: "saving" } }));
    try {
      localStorage.setItem("sgv_veh_draft", JSON.stringify({ formData }));
  ...
  const resetForm = () => {
    ...
    setUploadStatus("");
    setExistingOdometerKm("");
    try { localStorage.removeItem("sgv_veh_draft"); } catch { /* ignore */ }
    window.dispatchEvent(new CustomEvent("sgv:autosave", { detail: { status: "idle" } }));
  };
    ...
    try {
        ...
        if (!error && data) {
            ...
            const { data: invoicesData, error: invoicesError } = await supabase
              .from("invoices")
              .select("id, invoice_number, created_at, payment_mode, grand_total, paid_amount, outstanding_amount, note, labour, items")
            ...
            const { data: advanceRows } = await supabase
              ...
              .or(`note.ilike.%vehicle_id:${data.id}%,note.ilike.%car_id:${data.car_id}%`)
              .order("created_at", { ascending: false })
              .limit(200);
            ...
            (advanceRows || []).forEach((row) => {
              ...
              if (note.includes("invoice_advance_for:") && invoiceNumber) {
                advanceMap[invoiceNumber] = (advanceMap[invoiceNumber] || 0) + amount;
                advanceTotal += amount;
              } else if (note.includes("invoice_advance_use:")) {
                advanceTotal -= amount;
              }
            });
            setVehicleInvoiceAdvances(advanceMap);
            setVehicleAdvanceTotal(Math.max(advanceTotal, 0));
            setViewMode('profile');
        } else {
  ...
  const fetchVehicleForEdit = async (id: string) => {
    ...
    if (!error && data) {
        ...
        setExistingOdometerKm(data.odometer_km || "");
        setImageUrls({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (
      ...
      (!editingId && !formData.odometer_km.trim())
    ) {
      toast.error(!editingId && !formData.odometer_km.trim() ? "Please enter ODO reading" : "Please fill all required fields");
      return;
    }

    if (!/^[6-9]\d{9}$/.test(cleanedPhoneNumber)) {
    ...
    try {
      ...
      const hasOdometerUpdate =
        Boolean(editingId) &&
        formData.odometer_km.trim() &&
        formData.odometer_km.trim() !== existingOdometerKm.trim();
      if (hasOdometerUpdate) {
        noteParts.push(
          `Odometer updated: ${formData.odometer_km.trim()} km (${format(new Date(), "dd MMM yyyy")})`,
        );
      }
      const vehiclePayload = {
        ...
        odometer_km: formData.odometer_km.trim() || null,
  ...
  const handleDownloadInvoice = async (invoiceId: string) => {
    try {
      const { data, error } = await supabase
        .from("invoices")
        .select("invoice_number, created_at, payment_mode, grand_total, total_spare, total_labour, items, labour, note, vehicle_id")
        .eq("id", invoiceId)
        .maybeSingle();
      ...
      let invoiceVehicle = currentVehicle;
      if (!invoiceVehicle && data.vehicle_id) {
        const { data: vehicleData } = await supabase
          .from("vehicles")
          .select("owner_name, phone_number, car_id, vehicle_reg, make_model, entry_date, odometer_km")
          .eq("id", data.vehicle_id)
          .maybeSingle();
        if (vehicleData) {
          invoiceVehicle = vehicleData as VehicleWithProfile;
        }
      }
      ...
      const vehicleForPdf = invoiceVehicle
        ? {
            owner_name: invoiceVehicle.owner_name,
            phone_number: invoiceVehicle.phone_number,
            car_id: invoiceVehicle.car_id,
            vehicle_reg: invoiceVehicle.vehicle_reg,
            make_model: invoiceVehicle.make_model,
            entry_date: invoiceVehicle.entry_date,
            odometer_km: invoiceVehicle.odometer_km,
          }
        : {};
    ...
    >
                        ...
                        <td className="px-6 py-4 text-right pr-10">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              ...
                              className="p-1.5 hover:bg-violet-50 rounded-lg text-slate-300 hover:text-violet-600 transition-all  "
      ...
      {viewMode === 'profile' && currentVehicle && (
        <VehicleProfileView
          ...
          advanceTotal={vehicleAdvanceTotal}
          invoiceAdvances={vehicleInvoiceAdvances}
          noteDraft={noteDraft}
      ...
      {viewMode === 'new' && (
        <VehicleRegistrationForm
          ...
          existingOdometerKm={existingOdometerKm}
