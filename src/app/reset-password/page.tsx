...
export default function ResetPasswordPage() {
  ...
  React.useEffect(() => {
    ...
    const prepareRecoverySession = async () => {
      try {
        const { accessToken, refreshToken, tokenHash, type, source } = getRecoveryTokens();
        ...
        if (isMounted) {
          setIsReady(true);
          setError("");
          if (source === "welcome" || type === "recovery") {
  ...
  return (
    <div className="relative min-h-screen font-sans bg-[#0c0c14] text-[#e8e8f0] overflow-hidden selection:bg-indigo-500/30 selection:text-white">
      ...
      {showRecoveryPopup && isReady && !loading && !success ? (
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/70 px-6">
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#111126] p-6 text-center shadow-2xl">
            <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-emerald-500/20">
              <CheckCircle2 className="h-5 w-5 text-emerald-400" />
            </div>
            <h2 className="text-lg font-semibold text-white">Reset Link Verified</h2>
            <p className="mt-2 text-sm text-zinc-300">
              Your unique password reset link is active. Continue to set your new password now.
            </p>
            <button
              type="button"
              onClick={() => setShowRecoveryPopup(false)}
              className="mt-5 inline-flex h-10 w-full items-center justify-center rounded-xl bg-indigo-600 text-sm font-bold text-white transition hover:bg-indigo-500"
            >
              Continue
            </button>
          </div>
